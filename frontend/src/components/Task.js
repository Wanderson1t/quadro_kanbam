...
function Task() {
    ...
    const updateTodo = (data) => {
        axios.put(`http://localhost:5000/project/${projectId}/todo`, data)
            .then((res) => {
            }).catch((error) => {
                toast.error('Algo deu errado')
            })
    ...
    const handleDelete = (e, taskId) => {
        e.stopPropagation();
        axios.delete(`http://localhost:5000/project/${projectId}/task/${taskId}`)
            .then((res) => {
    ...
    return (
        <div className='px-12 py-6 w-full'>
            <div className="flex items-center justify-between mb-6">
                ...
                <BtnPrimary onClick={() => setAddTaskModal(true)}>Adicionar a fazer</BtnPrimary>
            </div>
            <DragDropContext
