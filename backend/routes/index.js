...
api.get('/project/:id', async (req, res) => {
    if (!req.params.id) res.status(422).send({ data: { error: true, message: 'Id é requerido' } })
    try {
...
api.post('/project', async (req, res) => {
    ...
    try {
        ...
        if (e.code === 11000) {
            return res.status(422).send({ data: { error: true, message: 'título deve ser único' } })
        } else {
            return res.status(500).send({ data: { error: true, message: 'erro do servidor' } })
        }
    }
...
api.post('/project/:id/task', async (req, res) => {
    ...
    if (!req.params.id) return res.status(500).send(`erro do servidor`);
...
api.get('/project/:id/task/:taskId', async (req, res) => {
    ...
    if (!req.params.id || !req.params.taskId) return res.status(500).send(`erro do servidor`);
    ...
    try {
        ...
        if (data[0].task.length < 1) return res.status(404).send({ error: true, message: 'registro não encontrado' })
        return res.send(data)
    } catch (error) {
